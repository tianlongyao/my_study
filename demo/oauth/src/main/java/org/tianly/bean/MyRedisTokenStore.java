...
public class MyRedisTokenStore implements TokenStore {
    public static final long ONE_HOUR_MILL = 3600 * 1000L;
        ...
        if(oAuth2Authentication != null){
            ...
            if (principal instanceof User) {
                ...
                if (blacklist.contains(userName)) {
                    ...
                    //如果一个小时内过期，则马上续期（周期任务1分钟执行）
                    if(token.getExpiration().before(new Date(System.currentTimeMillis()+ ONE_HOUR_MILL))){
                        System.out.println(token.getValue());
                    }else {//一个小时后过期（周期任务1小时执行）
                        System.out.println(token.getValue());
                    }
                }
            }
    ...
    public void storeAccessToken(OAuth2AccessToken token, OAuth2Authentication authentication) {
        ...
        byte[] approvalKey = serializeKey(UNAME_TO_ACCESS + getApprovalKey(authentication) + ":" + token.getValue());
        byte[] clientId = serializeKey(CLIENT_ID_TO_ACCESS + authentication.getOAuth2Request().getClientId() + ":" + token.getValue());
        RedisConnection conn = getConnection();
        try {
            conn.openPipeline();
            if (SPRING_DATA_REDIS_2_0) {
                try {
                    this.redisConnectionSet20.invoke(conn, accessKey, serializedAccessToken);
                    this.redisConnectionSet20.invoke(conn, authKey, serializedAuth);
                    this.redisConnectionSet20.invoke(conn, authToAccessKey, serializedAccessToken);
                } catch (Exception ex) {
            ...
            if (!authentication.isClientOnly()) {
                //使用key来存储值，避免同样的accesstonken，set 保存 很多记录
                conn.set(approvalKey, serializedAccessToken);
            }
            //避免每次续期 在Set中添加大量数据，修改为key
            conn.set(clientId, serializedAccessToken);
            ...
            if (refreshToken != null && refreshToken.getValue() != null) {
                ...
                if (SPRING_DATA_REDIS_2_0) {
                    try {
                        this.redisConnectionSet20.invoke(conn, refreshToAccessKey, auth);
    ...
    public void removeAccessToken(String tokenValue) {
        ...
        try {
            ...
            if (authentication != null) {
                ...
                byte[] unameKey = serializeKey(UNAME_TO_ACCESS + getApprovalKey(authentication) + ":" + tokenValue);
                byte[] clientId = serializeKey(CLIENT_ID_TO_ACCESS + authentication.getOAuth2Request().getClientId() + ":" + tokenValue);
                conn.openPipeline();
